...
// Visibility -> a GridLength for the row that holds it.
//
...
public sealed class VisibilityToStarHeight : IValueConverter
{
    ...
    public object Convert(object? value, Type t, object? p, CultureInfo c)
    {
        var visible = value is Visibility state && state == Visibility.Visible;
        if (visible)
        {
...
// LogEntry -> its single formatted line. Formatting lives on the entry so the
// on-screen text and the file text cannot drift apart.
public sealed class LogLineConverter : IValueConverter
{
    public object Convert(object? value, Type t, object? p, CultureInfo c)
    {
        if (value is Core.Diagnostics.LogEntry entry)
        {
            return entry.Format();
        }
