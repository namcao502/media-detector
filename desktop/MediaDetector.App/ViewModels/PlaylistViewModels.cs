...
public sealed partial class PlaylistTrackViewModel : ObservableObject
{
    ...
    // Set by the owning panel so the row can read the shared CleanNames /
    // ShowOriginalTitles state without holding a back-reference.
    ...
    private void Commit()
    {
        ...
        CustomName = trimmed.Length == 0 || trimmed == GeneratedName(GetClean()) ? null : trimmed;
        IsEditing = false;
    }
...
public sealed partial class PlaylistPanelViewModel : StreamingViewModel
{
    ...
    public int RenamedCount => Tracks.Count(track => track.CustomName != null);
    ...
    private async Task DownloadAllAsync()
    {
        ...
        var request = new PlaylistDownloadRequest(
            ...
            GetOutputDir(),
            GetCleanNames(),
            Tracks.Where(track => track.CustomName != null)
                  .ToDictionary(track => track.Index, track => track.CustomName!),
            Concurrency);
    ...
    private void Apply(DownloadLine line)
    {
        switch (line)
        {
            case ItemLine item:
            {
                ...
                // Anchored on the earliest track still running rather than on the
                // one that just started: with several workers the newest start can
                // be far down the list, and following it would yank the view about.
                ScrollIntoView?.Invoke(Tracks.FirstOrDefault(row => row.IsActive) ?? track);
                break;
            }
    ...
    // Called by MainViewModel when the shared CleanNames toggle flips, and after a
    // rename reset. DisplayName depends on state this view model owns, so the
